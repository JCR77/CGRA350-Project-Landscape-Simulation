...
WaterSurface::WaterSurface(float size, float height) : height_(height)
{
    ...
    // create square plane
    mesh_vertex v0 = {.pos = vec3(-half_size, height, -half_size), .norm = vec3(0, 1, 0), .uv = vec2(0, 1)};
    mesh_vertex v1 = {.pos = vec3(half_size, height, -half_size), .norm = vec3(0, 1, 0), .uv = vec2(1, 1)};
    mesh_vertex v2 = {.pos = vec3(-half_size, height, half_size), .norm = vec3(0, 1, 0), .uv = vec2(0, 0)};
    mesh_vertex v3 = {.pos = vec3(half_size, height, half_size), .norm = vec3(0, 1, 0), .uv = vec2(1, 0)};
...
void WaterSurface::draw(const glm::mat4 &view, const glm::mat4 proj)
{
    ...
    glUniformMatrix4fv(glGetUniformLocation(shader_, "uModelViewMatrix"), 1, false, value_ptr(view));
