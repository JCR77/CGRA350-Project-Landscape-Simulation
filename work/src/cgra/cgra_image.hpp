...
namespace cgra
{
    ...
    struct rgba_image
    {
        ...
        /**
         * File names in the face order right, left, top, bottom, front, back
         */
        static GLuint createCubeMapTexture(const std::vector<std::string> &file_names)
        {
            GLuint tex;
            glGenTextures(1, &tex);
            glActiveTexture(GL_TEXTURE0);
            ...
            for (glm::uint i = 0; i < file_names.size(); i++)
            {
